...
from space_manager.helpers import OsOperations, Usage
from space_manager.shared_objects import Category, DataFolder, LinkFolder, Volume
...
class Distributer(object):
    ...
    def distribute_from_link_folders(self):
        ...
        """
        log_name = "distribute_from_link_folders"
        ...
        self._logger.divider()
        self._logger.log('start of %s for %s' % (log_name, folder_name_prefix), 0, 1)
    ...
    def distribute_from_data_folders(self):
        log_name = "distribute_from_data_folders"

        self._logger.divider()
        self._logger.log('start of %s for %s' % (log_name, self.category.prefix), 0, 1)
    ...
    def __end(self, name: str):
        self._logger.log('End of %s for %s' % (name, self.category.prefix), -1)
        self._logger.divider()

    def __find_folder_to_move(self, emptiest_free: int, fullest_free, parent_folder: DataFolder) -> str:
    ...
    def __enough_free_space(self, emptiest_free: int, fullest_free: int) -> bool:
    ...
    def __fullest_threshold_met(self, fullest_usage: Usage) -> bool:
    ...
    def __condition_not_met(self):
        self._logger.log("condition not met. ending.")

    def __get_folder_paths_to_move(
            self,
            folder_name_prefix: str,
            source_volumes: List[Volume],
            minimum_age) -> List[str]:
        folder_paths = []
